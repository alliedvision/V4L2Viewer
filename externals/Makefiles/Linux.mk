...
DEFAULT_CC              = gcc
CC                      = $(DEFAULT_CC)
ifeq ($(FLOATABI),ignore)
CC_THUMB =
CC_ARMARCH =
CC_FLOATABI =
STRIP = strip --strip-unneeded $(1)
else ifneq ($(CC),$(DEFAULT_CC))
CC_FLOATABI = -mfloat-abi=$(FLOATABI)
ifeq ($(FLOATABI),soft)
CC_THUMB = -marm
CC_ARMARCH = -march=armv4t
else ifeq ($(FLOATABI),hard)
CC_THUMB = -mthumb
ifeq ($(WORDSIZE),32)
CC_ARMARCH = -march=armv7
endif
ifeq ($(WORDSIZE),64)
CC_ARMARCH = -march=armv8-a
endif
endif
endif
ifeq ($(ARCH),x86)
...
MACRO_CC                = $(CC) $(foreach dir,$(3),-I$(dir)) $(foreach def,$(4),-D$(def)) $(if $(filter CC_OPTION_VISIBLE,$(6)),,-fvisibility=hidden) -fPIC $(CC_CONFIG_FLAGS) $(CC_ARCH) $(CC_FLOATABI) $(CC_THUMB) $(CC_ARMARCH) -o $(1) -c $(2) $(foreach pkg,$(5),$(shell pkg-config --cflags $(pkg)))
...
CXX_FLOATABI =
STRIP = strip --strip-unneeded $(1)
else ifneq ($(CXX),$(DEFAULT_CXX))
CXX_FLOATABI = -mfloat-abi=$(FLOATABI)
ifeq ($(FLOATABI),soft)
CXX_THUMB = -marm
CXX_ARMARCH = -march=armv4t
else ifeq ($(FLOATABI),hard)
CXX_THUMB = -mthumb
ifeq ($(WORDSIZE),32)
CXX_ARMARCH = -march=armv7
endif
ifeq ($(WORDSIZE),64)
CXX_ARMARCH = -march=armv8-a
endif
endif
endif
ifeq ($(ARCH),x86)
...
MACRO_CXX               = $(CXX) -std=gnu++98 $(foreach dir,$(3),-I$(dir)) $(foreach def,$(4),-D$(def)) $(if $(filter CXX_OPTION_VISIBLE,$(6)),,-fvisibility=hidden) -fPIC $(CXX_CONFIG_FLAGS) $(CXX_ARCH) $(CXX_FLOATABI) $(CXX_THUMB) $(CXX_ARMARCH) -o $(1) -c $(2) $(foreach pkg,$(5),$(shell pkg-config --cflags $(pkg)))
...
MACRO_LINK_BINARY       = $(CXX) $(CXX_ARCH) $(CXX_FLOATABI) $(CXX_THUMB) $(CXX_ARMARCH) $(if $(6),-Wl$(CONST_COMMA)--retain-symbols-file=$(6) $(foreach symbol,$(shell cat $(6)),-Wl$(CONST_COMMA)-u$(CONST_COMMA)$(symbol))) -o $(1) $(2) $(foreach dir,$(4),-L$(dir)) $(foreach dir,$(7),-Wl$(CONST_COMMA)-rpath$(CONST_COMMA)$(dir)) $(foreach dir,$(8),-Wl$(CONST_COMMA)-rpath-link$(CONST_COMMA)$(dir)) $(foreach lib,$(3),$(if $(findstring STATIC_,$(lib)),$(patsubst STATIC_%,-Wl$(CONST_COMMA)-Bstatic -l%, $(lib)),-Wl$(CONST_COMMA)-Bdynamic -l$(lib))) $(patsubst %,-L%,$(subst :, ,$(ADD_LIBRARY_PATH))) $(foreach pkg,$(5),$(shell pkg-config --libs $(pkg))) -Wl,--no-undefined
